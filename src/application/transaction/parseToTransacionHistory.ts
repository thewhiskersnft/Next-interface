import { HexAddress, TxHistoryInfo } from "@/interfaces";

export default function parseToTransacionHistory(
  owner: HexAddress,
  rawHistory: any
): TxHistoryInfo[] {
  function spawnTitleFromOld(item: TxHistoryInfo): TxHistoryInfo {
    ...
    const isLiquidity = item.description
      ?.toLowerCase()
      .includes("add liquidity");
